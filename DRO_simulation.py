...
class Simulation:
    ...
    def __init__(self, model, Q, Qf, R, mu, x_init, beta = 0.95, N_sample = 10, i_th_state = 1, i_state_ub = 0.05, epsilon = 1,
                 sin_const = 1, N_sim=80, mode = "collect",data_set = None, N_sample_max = None):
        ...
        if mode is "collect":
            self.x_sim, self.u_sim = self.simulation_collect(x_init,N_sim, data_set, N_sample_max)
        elif mode is "gene":
    ...
    def simulation_collect(self,x_init,N_sim, data_set, N_sample_max):
        Ak = self.model.Ak
        Bk = self.model.Bk
        C = self.model.C
        D = self.model.D
        ...
        for i in range(N_sim):
            ...
            opt_problem = Opt_problem(self.model, Q, Qf, R, mu, beta=beta, N_sample=N_sample, i_th_state=i_th_state,
                                      i_state_ub=i_state_ub, epsilon=epsilon, sin_const=sin_const, collect = True, data_set = data_set)
